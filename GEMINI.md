...
The analysis step (Step 4 in the daily pick script) is performed **directly by Gemini CLI** in the conversation — not by calling the Anthropic API from the script. The script should gather all fixture/odds/form data and return it to Gemini CLI, which then does the analysis and writes the result. There is no `ANTHROPIC_API_KEY` required in `.env.local`.
...
**VERCEL_ORG_ID and VERCEL_PROJECT_ID**: These are written automatically to `.vercel/project.json` after the first deploy. Gemini CLI will extract them and add them to `.env.local` during setup.
...
### Step 4 — Value analysis via Gemini CLI

Gemini CLI (in the conversation) reads `data/pending-analysis.json` and performs value analysis directly. The script does NOT call the Anthropic API — it just writes the pending data file and instructs Gemini to analyse it.
...
- Invoke Gemini CLI's frontend design capabilities fully — this should look considered and distinctive
...
- Gemini CLI then reads that file, performs value analysis, and writes the dated JSON brief

After Gemini writes the brief, commit and push:
```bash
